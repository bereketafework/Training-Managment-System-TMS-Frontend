<template>
  ...
  <div class="flex flex-row mt-2">
        ...
        <v-form ref="form" @submit.prevent="submit" v-model="formValid">
          <v-container fluid class="border-[1px] border-gray-200 !w-[900px]">
            <v-row class="!flex !flex-row">
              ...
              <v-col cols="8" sm="4">
                <v-text-field v-model="form.email" :rules="[rules.required, rules.email]" color="black darken-2" label="Email" type="email"></v-text-field>
              </v-col>
              <v-col cols="8" sm="4">
    ...
    <v-spacer></v-spacer>
    <span class="flex justify-center border-b-[1px] text-3xl">List of Participants</span>
    <v-spacer></v-spacer>
      ...
      <v-card-text>
        <v-form ref="filterForm" @submit.prevent="applyFilter">
          <v-container>
            <v-row>
              <v-col cols="12" sm="6">
                <v-text-field v-model="filter.first_name" label="First Name"></v-text-field>
              </v-col>
              <v-col cols="12" sm="6">
                <v-text-field v-model="filter.last_name" label="Last Name"></v-text-field>
              </v-col>
              <v-col cols="12" sm="6">
                <v-text-field v-model="filter.email" label="Email"></v-text-field>
              </v-col>
              <v-col cols="12" sm="6">
                <v-text-field v-model="filter.phone" label="Phone" type="number"></v-text-field>
              </v-col>
            </v-row>
          </v-container>
          <v-card-actions>
  ...
  computed: {
    filteredParticipants() {
      ...
      if (this.filter.first_name) {
        results = results.filter(item => item.first_name.includes(this.filter.first_name));
      }
      if (this.filter.last_name) {
        results = results.filter(item => item.last_name.includes(this.filter.last_name));
      }
      if (this.filter.email) {
        results = results.filter(item => item.email.includes(this.filter.email));
      }
      if (this.filter.phone) {
        results = results.filter(item => item.phone == this.filter.phone);
      }
      return results;
    },
